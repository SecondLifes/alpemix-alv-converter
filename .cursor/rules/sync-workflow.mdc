...
and removes any generated file, wrapper or link whose source was deleted. If
a skill's folder name collides with a hand-authored file under
`.agents/commands/`, the script skips generating a wrapper for it and prints
a warning — hand-authored commands always win. Never hand-edit files inside
`.claude/rules/`, `.cursor/rules/`, `.claude/commands/` or `.claude/skills/`
directly — they will be silently overwritten on the next run and any change
made only there will be lost.
